...
Dockerfiles are scripts that define images and they are built.  They can be used to define user, what volumes are mounted from the host, what install software to install in the image, etc.

In an admittedly imperfect attempt to make this more relatable to software developers, we're going to intersperse our explanation with some analogies between images and containers and Object Oriented programming concepts, which should be familiar to the average application developer.  Dockerfiles can loosely be compared to class files, and images to classes.  Again, this analogy is imperfect, but we believe it will make it easier to internalize what an image and Dockerfile are faster.
...
By convention, name your file "Dockerfile" without an extension.

  podman build .

The above with automatically look for a file named "Dockerfile" to build in the current work directory.  If for some reason you wish to deviate from convention, you can use the flag `-f` or `--file`, which can take a path or a URL as an argument; e.g.
...
=== Image layers
In general, every instruction in a Dockerfile adds a _layer_ to the new image, which in turn increases the image size.  An image layer represents a set of changes to the image's file system.  Layers are cached for faster builds and downloads.
