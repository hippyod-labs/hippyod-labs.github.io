...
===== Image repositories do **NOT** support version control
...
CAUTION: Just because containers ensure the environment your application is deployed to is always the same, that does not mean it will have the same configuration; e.g. development and production are rarely deployed with the same configuration, the .  Containers can drastically minimize the problem, but cannot eliminate the issue completely.
